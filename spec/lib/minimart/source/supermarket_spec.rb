require 'spec_helper'

describe Minimart::Source::Supermarket do

  let(:url) { 'https://fakechefsupermarket.com' }
  let(:raw_cookbooks) do
    {
      mysql: { versions: ["3.5", "4.0"] },
      yum:   { versions: ["3.4"] }
    }
  end

  subject { Minimart::Source::Supermarket.new(url, raw_cookbooks) }

  describe '::new' do
    it 'should properly assign the url' do
      expect(subject.url).to eq url
    end

    it 'should build the proper dependencies' do
      dependencies = subject.dependencies
      expect(dependencies.any? { |c| c.name == 'mysql' && c.version = '3.5' }).to eq true
      expect(dependencies.any? { |c| c.name == 'mysql' && c.version = '4.0' }).to eq true
      expect(dependencies.any? { |c| c.name == 'yum' && c.version = '3.4' }).to eq true
    end

    it 'should build a universe' do
      expect(subject.universe).to be_a Minimart::Source::Universe
    end

    it 'should pass the proper url to the universe' do
      expect(subject.universe.base_url).to eq url
    end
  end

  describe '#download_cookbooks' do
    let(:mysql_dependency) { {name: 'mysql', version: '4.5'} }
    let(:yum_dependency) { {name: 'yum', version: '5.0'} }
    let(:block) { Proc.new { true } }

    before(:each) do
      subject.dependencies = [mysql_dependency, yum_dependency]
    end

    it 'should download the dependencies' do
      expect(subject).to receive(:download_cookbook).with mysql_dependency, 'fake_dir', &block
      expect(subject).to receive(:download_cookbook).with yum_dependency, 'fake_dir', &block
      subject.download_cookbooks('fake_dir', &block)
    end
  end

  describe '#download_cookbook' do
    pending do

      let(:mock_cookbook) do
        Hashie::Mash.new(
          name: 'mysql',
          version: '3.5.0',
          download_url: 'http://sprkmrkt.com/mysql')
      end

      let(:mock_archive) { double('archive') }
      let(:block) { Proc.new { true } }

      before(:each) do
        allow(subject.universe).to receive(:find_cookbook_for_requirements).and_return mock_cookbook
      end

      it 'should download the cookbook from the source' do
        expect(Minimart::Utils::Http).to receive(:get_binary).with(anything, mock_cookbook.download_url)
        subject.download_cookbook mock_cookbook, 'fake_dir', &block
      end

      it 'should pass the cookbook and archive to the block passed in' do
        allow(Minimart::Utils::Http).to receive(:get_binary).and_return mock_archive
        expect(block).to receive(:call).with(mock_cookbook, mock_archive)
        subject.download_cookbook mock_cookbook, 'fake_dir', &block
      end
    end
  end

  describe '#resolve_dependency' do
    pending
  end
end
